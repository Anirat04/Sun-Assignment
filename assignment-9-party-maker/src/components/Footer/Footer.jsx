...
const Footer = () => {
    const { servicesData } = useContext(ProviderContext)
    console.log(servicesData)

    return (
        <div className="bg-base-300">
            <div className="max-w-[1200px] mx-auto">
                <Link to="/"><img className="p-[20px] ml-[20px]" src="https://i.ibb.co/t4qxJj5/logo-Black.png" alt="" /></Link>
                <footer className="footer p-10 pt-0 text-base-content">
                    <nav>
                        <header className="footer-title">Services</header>
                        {
                            servicesData.map(serviceData => (
                                <Link to={`/details/${serviceData.id}`} key={serviceData.id} className="link link-hover">{serviceData.title}</Link>
                            ))
                        }
                    </nav>
                    <nav>
                ...
                <footer className="footer px-10 py-4 border-t border-gray-300 text-base-content">
                    <aside className="items-center grid-flow-col">
                        ...
                        <p>ACME Industries Ltd. <br />Providing reliable event management services since 1992</p>
                    </aside>
